...
void Master::test_master() 
{
    ...
    switch (identify) {
        case  0:
			...
			//WRITING 1st VALUE
			data = 0x00000010;
            cout << "[Master0]: will send 1st value ("<<data<<") for writing at Slave0"<<endl<<endl;
			burst= 0x002;
            //passo("m0",0);
            ...
            if ( wr_resp_port.read() == 1 )
                cout << "[Master0]: Write OK 1" << endl<<endl;
            else
                cout << "[Master0]: Write error 1" << endl<<endl;
            //passo("m0,wr_resp_port.read",2);
			
			//READING VALUE WRITTEN BEFORE
			cout <<"[Master0]: will read the value from memory written before"<<endl<<endl;
			burst= 0x001;
            cmd_port->write( CMD_LW );
            addr_port->write( address0 );
			burst_port.write( burst );
            data = rd_data_port->read();
            cout << "[Master0]: Result = " << data << endl<<endl;

			//WRITING 2nd VALUE

            data = 0x00000020;//32 em inteiro
            cout << "[Master0]: will send 2nd value ("<<data<<") for writing at Slave0"<<endl<<endl;
			burst= 0x002;
            wr_data_port.write( data );
            cmd_port.write( CMD_SW ); // store word
            addr_port.write( address0 );
			burst_port.write(burst);
            if ( wr_resp_port.read() == 1 )
                cout << "[Master0]: Write OK 2" << endl<<endl;
            else
                cout << "[Master0]: Write error 2" << endl<<endl;
        ...
        case 1:
            ...
            //WRITING 1st VALUE
            data = 0x00000012;
            cout << "[Master1]: will send 1st value ("<<data<<") for writing at Slave0"<<endl<<endl;
			burst= 0x002;
            //passo("m0",0);
            wr_data_port.write( data );
            //passo("m0,wr_data_port.write",1);
			...
			burst_port.write(burst);
            if ( wr_resp_port.read() == 1 )
                cout << "[Master1]: Write OK 1" << endl<<endl;
            else
                cout << "[Master1]: Write error 1" << endl<<endl;
            //passo("m0,wr_resp_port.read",2);
			//READING VALUE WRITTEN BEFORE
			cout <<"[Master1]: will read the value from memory written before"<<endl<<endl;
			burst= 0x001;
            cmd_port->write( CMD_LW );
            addr_port->write( address3 );
			burst_port.write( burst );
            data = rd_data_port->read();
            cout << "[Master1]: Result = " << data << endl<<endl;

			//WRITING 2nd VALUE
			data = 0x00000022;//32 em inteiro
            cout << "[Master1]: will send 2nd value ("<<data<<") for writing at Slave0"<<endl<<endl;
			burst= 0x002;
            wr_data_port.write( data );
            cmd_port.write( CMD_SW ); // store word
            addr_port.write( address3 );
			burst_port.write(burst);
            if ( wr_resp_port.read() == 1 )
                cout << "[Master1]: Write OK 2" << endl<<endl;
            else
                cout << "[Master1]: Write error 2" << endl<<endl;
